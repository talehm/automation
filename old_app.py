...
from image_old import dropbox_unused, generator
import json
import os
import re
import time
...
import old_socials.social as social
...
def create_house(cat_id):
    ...
    while True:
        try:
            ...
            articles = posts_unused.fetch_by_category(cat_id)
            # print(articles)
            for article in articles:
                ...
                imgur_url = dropbox_unused.upload(img_local_url)
                print(imgur_url)
...
def create_definitions_hold(lang):
    ...
    try:
        ...
        for index, definition in enumerate(definitions):
            if index >= 10:
                break
            item = posts_unused.fetch_by_id(post_type, definition["id"])
            if item == None:
                continue
...
def create_definitions(lang):
    ...
    posts_unused.fetch_pins(post_type)
...
def create_puzzles():
    ...
    try:
        ...
        for number in range(1, (total_riddles // number_of_riddles_per_article) + 2):
            ...
            item = posts_unused.create_post(post_type, riddles_chunk, number)
...
def update_metadata():
    posts_unused.update_metadata_all("definition")
    return "Done"


@app.route("/definition/links")
def create_links():
    posts_unused.create_links("definition")
    return "Done"
